...
int main(int argc, char **argv)
{
	...
	while ((pos < argc) && (*(argv[pos]) == '-'))
	{
		switch (*(argv[pos] + 1))
		{
			case 'l':
				lawful = 1;
				console_log("Lawful mode selected.");
			break;
			case 'd':
				if (*(argv[pos] + 2))
					dir = argv[pos] + 2;
				...
				{
					console_log("Directory arg expected after option -d.");
					exit(0);
				}
			break;
			case 's':
				no_specials = 1;
				console_log("Suppressing assignment of special routines.");
			break;
			default:
...
int run_the_game(int port)
{
	...
	console_log("Signal trapping.");
	signal_setup();

	console_log("Opening mother connection.");
	s = init_socket(port);

	if (lawful && load() >= 6)
	{
		console_log("System load too high at startup.");
		coma(0);
	}

	boot_db();

	console_log("Entering game loop.");
	...
	console_log("Normal termination of game.");
}
...
int process_input(struct descriptor_data *t)
{
	...
	do
	{
		if ((thisround = read(t->descriptor, t->buf + begin + sofar, 
			...
			{
				console_log("EOF encountered on socket read.");
				return(-1);
			}
	}
...
void close_sockets(int s)
{
	console_log("Closing all sockets.");
...
void close_socket(struct descriptor_data *d)
{
	...
	if (d->character)
		...
		console_log("Losing descriptor without char.");
...
#define COMA_SIGN "DikuMUD is currently inactive due to excessive load on the host machine."
...
void coma(int s)
{
	...
	console_log("Entering comatose state.");
	...
	do
	{
		...
		if (FD_ISSET(s, &input_set))
		{
			if (load() < 6)
			{
				console_log("Leaving coma with visitor.");
				sigsetmask(0);
				return;
			}
		...
		if (workhours())
		{
			console_log("Working hours collision during coma. Exit.");
			exit(0);
		}
	}
	while (load() >= 6);

	console_log("Leaving coma.");
	sigsetmask(0);
}
			...
			for (strp = str, point = buf;;)
				if (*strp == '$')
				{
					switch (*(++strp))
					{
						...
						case '$': i = "$"; break;
						default:
							console_log("Illegal $-code to act():");
							console_log(str);
							break;
					}
					while (*point = *(i++))
