...
void night_watchman(void)
{
	...
	if ((t_info->tm_hour == 8) && (t_info->tm_wday > 0) &&
		...
		if (t_info->tm_min > 50)
		{
			console_log("Leaving the scene for the serious folks.");
...
void check_reboot(void)
{
	...
	if ((t_info->tm_hour + 1) == REBOOT_AT && t_info->tm_min > 30)
		if (boot = fopen("./reboot", "r"))
		{
			if (t_info->tm_min > 50)
			{
				console_log("Reboot exists.");
				fread(&dummy, sizeof(dummy), 1, boot);
				if (!feof(boot))   /* the file is nonepty */
				{
					console_log("Reboot is nonempty.");
					if (system("./reboot"))
					{
						console_log("Reboot script terminated abnormally");
...
char *nogames(void)
{
	...
	{
		console_log("/usr/games/nogames exists");
		fgets(text,200,fl);
		return(text);
		fclose(fl);
	}
