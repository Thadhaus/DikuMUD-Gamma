...
/*			ch->player.time.birth += mod; */
		...
		default:
			console_log("Unknown apply adjust attempt (handler.c, affect_modify).");
			break;
...
void affect_remove( struct char_data *ch, struct affected_type *af )
{
	...
	if (ch->affected == af) {
		...
		if (hjp->next != af) {
			console_log("FATAL : Could not locate affected_type in ch->affected. (handler.c, affect_remove)");
			exit(1);
		}
...
void char_from_room(struct char_data *ch)
{
	...
	if (ch->in_room == NOWHERE) {
		console_log("NOWHERE extracting char from room (handler.c, char_from_room)");
		exit(1);
	}
...
void equip_char(struct char_data *ch, struct obj_data *obj, int pos)
{
	...
	if (obj->carried_by) {
		console_log("EQUIP: Obj is carried_by when equip.");
		return;
	}

	if (obj->in_room!=NOWHERE) {
		console_log("EQUIP: Obj is in_room when equip.");
		return;
	}
	...
	if ((IS_OBJ_STAT(obj, ITEM_ANTI_EVIL) && IS_EVIL(ch)) ||
		...
		if (ch->in_room != NOWHERE) {
			...
			console_log("ch->in_room = NOWHERE when equipping char.");
		}
	}
...
void extract_char(struct char_data *ch)
{
	...
	if (ch->in_room == NOWHERE) {
		console_log("NOWHERE extracting char. (handler.c, extract_char)");
		exit(1);
	}
   ...
   if(ch->desc) {
	...
	else
	{
		...
		if(k)
			...
			console_log("Trying to remove ?? from character_list. (handler.c, extract_char)");
			abort();
		}
	}
...
struct obj_data *create_money( int amount )
{
	...
	if(amount<=0)
	{
		console_log("ERROR: Try to create negative money.");
		exit(1);
	}
